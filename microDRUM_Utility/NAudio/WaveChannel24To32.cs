...
namespace NAudio.Wave
{
    ...
    /// 32 bit output and 24 bit input
    /// It's output is always stereo
    /// The input stream can be panned
    /// </summary>
    public class WaveChannel24To32 : AudioEffectWaveStream, ISampleNotifier
    {
        ...
        /// <summary>
        /// Creates a WaveChannel24To32 with default settings
        /// </summary>
        /// <param name="sourceStream">The source stream</param>
        public WaveChannel24To32(WaveStream sourceStream)
            :
        ...
        public override int Read(byte[] destBuffer, int offset, int numBytes)
        {
            ...
            if (bytesWritten < numBytes)
            {
                ...
                {
                    int sourceBytesRequired = (numBytes - bytesWritten)*3 / 8;
                    ...
                    bytesWritten += ((read * 8)/3);
                }
                else
                {
                    int sourceBytesRequired = ((numBytes - bytesWritten) * 3) / 4;
                    byte[] sourceBuffer = GetSourceBuffer(sourceBytesRequired);
                    int read = sourceStream.Read(sourceBuffer, 0, sourceBytesRequired);
                    AdjustVolume(destBuffer, offset + bytesWritten, sourceBuffer, read);
                    bytesWritten += ((read * 4) / 3);
                }
            }
        ...
        private unsafe void MonoToStereo(byte[] destBuffer, int offset, byte[] sourceBuffer, int bytesRead)
        {
                ...
                pSourceBuffer = &sourceBuffer[0])
            {
                ...
                {
                    int Add = pSourceBuffer[(n * 3) + 2] > 0x7f ? 0xff : 0x00;
                    int Buffer = (pSourceBuffer[n * 3]) | (pSourceBuffer[(n * 3) + 1] << 8) | (pSourceBuffer[(n * 3) + 2] << 16) + (Add << 24);
                    pfDestBuffer[n*2] = Buffer * leftVolume;
                    pfDestBuffer[n*2 + 1] = Buffer * rightVolume;
        ...
        private unsafe void AdjustVolume(byte[] destBuffer, int offset, byte[] sourceBuffer, int bytesRead)
        {
                ...
                pSourceBuffer = &sourceBuffer[0])
            {
                ...
                {
                    int lAdd = pSourceBuffer[(n * 3) + 2] > 0x7f ? 0xff : 0x00;
                    int rAdd = pSourceBuffer[(n + 1) * 3 + 2] > 0x7f ? 0xff : 0x00;
                    int leftBuffer = (pSourceBuffer[n * 3]) | (pSourceBuffer[(n * 3) + 1] << 8) | (pSourceBuffer[(n * 3) + 2] << 16) +(lAdd << 24);
                    int rightBuffer = (pSourceBuffer[(n + 1) * 3]) | (pSourceBuffer[(n + 1) * 3 + 1] << 8) | (pSourceBuffer[(n + 1) * 3 + 2] << 16) +(rAdd << 24);
